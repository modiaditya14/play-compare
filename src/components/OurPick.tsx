...
export function OurPick({ app }: { app: PlayApp }) {
  return (
    <div className="panel-emboss ancient-scroll-panel relative overflow-hidden rounded-3xl border border-white/5 p-4 sm:p-6">
      <div className="badge-gloss-gold relative -right-[6] -top-6 flex h-10 sm:h-12 w-20 sm:w-24 items-center justify-center rounded-full text-xs sm:text-[15px] font-bold uppercase tracking-wide text-ink badge-skewed">
        <span className="kinetic badge-skewed-text text-center leading-tight">
          Our<br />Pick
        </span>
      </div>
      ...
      <p className="mt-3 sm:mt-4 text-xs sm:text-sm leading-relaxed text-chalk/70">
        Highest-scoring app in {app.category} once we weigh features and rating —
        This is a custom logic calculated pick.
      </p>
    </div>
  );
}
